...
export function ClientFormPro({ onAddClient }: { onAddClient: (data: FormData) => void }) {
  ...
  const spinTransition = {
    loop: Infinity, // Para que continue girando
    ease: "linear", // Movimento constante
    duration: 1 // Duração da rotação
  };
      ...
      <SectionTitle onClick={() => toggleSection('address')}>
        ...
        ?
          <motion.div 
            // initial="collapsed"
            animate="spin" 
            // animate={isContactsSectionExpanded ? "expanded" : "collapsed"}
            variants={iconVariants}
            
            // transition={{ duration: 0.3 }} // Duração da animação de rotação
